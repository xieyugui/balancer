...
MakeBalancerInstance(const char *opt) {
    ...
    if (len == lengthof("roundrobin") && strncmp(opt, "roundrobin", len) == 0) {
        return MakeRoundRobinBalancer(end ? end + 1 : nullptr, BALANCER_ROUND);
    } else if (len == lengthof("follow") && strncmp(opt, "follow", len) == 0) {
        return MakeHeaderBalancer(end ? end + 1 : nullptr, BALANCER_HEADER);
    } else {
...
static TSReturnCode send_response_handle(TSHttpTxn txnp, BalancerTargetStatus *targetstatus) {
    ...
    if (targetstatus->target_id >= 0 &&
        (!targetstatus->target_down or (targetstatus->target_down && targetstatus->is_down_check))) {
        ...
        TSHttpTxnErrorBodySet(txnp, TSstrdup("503 Source Service Unavailable!"),
                              sizeof("503 Source Service Unavailable!") - 1, NULL);
        return TS_ERROR;
    }
...
TSReturnCode TSRemapNewInstance(int argc, char *argv[], void **instance,
    ...
    static const struct option longopt[] = {{const_cast<char *>("policy"),
                                            ...
                                            {const_cast<char *>("https"), no_argument,       0, 's'},
                                            {const_cast<char *>("open"),  no_argument,       0, 'o'},
                                            {0, 0,                                           0, 0}};
    ...
    for (i = optind; i < argc; ++i) {
        BalancerTarget *target = balancer->make_balancer_target(argv[i]);
        target->id = s_count;
        s_count++;
...
void TSRemapDeleteInstance(void *instance) {
    ...
    static_cast<BalancerInstance *>(instance)->release();
}
